...
for cidade in cidades:
    for cap in caps:
        ...
        anosg = []
        for ano in anos:
            df = pd.read_csv(f"src\Gráficos\.Tabelas Consulta\dados cid10 {ano}.csv", sep=";", encoding="utf-8")
            ...
            if len(anosg) == 4:
                ...
                fig = px.bar(df, x='Ano', y='Quantidade de Internações', color='Ano', title=f'Casos de Internações {capsdesc[caps.index(cap)]} - {cidade[7:]}')
                fig.show()
